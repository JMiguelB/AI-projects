...
export const Header: React.FC<HeaderProps> = ({ currentDate, view, calendarName, onNext, onPrev, onToday, onViewChange }) => {
  ...
  return (
    <header className="p-4 bg-white dark:bg-slate-800 dark:border-b dark:border-slate-700 shadow-md flex items-center justify-between">
      <div className="flex items-center gap-4">
        <CalendarIcon className="h-8 w-8 text-[var(--primary-600)]" />
        <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-100">{calendarName}</h1>
      </div>
      <div className="flex items-center gap-4">
        <button
          onClick={onToday}
        ...
        <div className="flex items-center border border-slate-300 dark:border-slate-600 rounded-md shadow-sm">
        ...
        <div className="flex items-center">
          ...
          <h2 className="w-auto min-w-[15rem] text-center text-lg font-semibold text-slate-700 dark:text-slate-300">
            {getHeaderText()}
          </h2>
